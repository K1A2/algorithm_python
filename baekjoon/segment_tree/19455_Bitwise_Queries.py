...
tree = [[(1 << 30) - 1, 0, 0, (1 << 30) - 1, 0] for _ in range(n * 4)]


def init(node, left, right):
    if left == right:
        num = data[left - 1]
        tree[node] = [num, num, num, (1 << 30) - 1, 0]
        return
    mid = (left + right) // 2
...
def propagation(node):
    if tree[node][3] == (1 << 30) - 1 and tree[node][4] == 0:
        return
...
def query_min(node, start, end, left, right):
    ...
    mid = (left + right) // 2
    return min(
        query_min(node * 2, start, end, left, mid),
        query_min(node * 2 + 1, start, end, mid + 1, right)
    )
...
for _ in range(int(input())):
    ...
    if q[0] == '&':
        _, l, r, x = q
        update_and(1, int(l), int(r), 1, n, int(x))
    elif q[0] == '|':
        _, l, r, x = q
        update_or(1, int(l), int(r), 1, n, int(x))
    elif q[0] == '?':
        _, l, r = q
        sys.stdout.write(f'{query_min(1, int(l), int(r), 1, n)}\n')
